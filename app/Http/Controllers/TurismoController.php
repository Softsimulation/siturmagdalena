...
class TurismoController extends Controller
{
    public function getIndex(Request $request){
        ...
        return ['query' => $this->queHacerData($search = null)['query']];
        return view('turismo.Index', ['query' => $this->queHacerData($search = null)['query'], 'success' => true]);
    }
    ...
    private function queHacerData ($search){
        ...
        if ($search == null){
            $query = DB::select("SELECT actividades.id AS id,  
                  ...
                  1 AS tipo,
                  ARRAY[1, 2, 3] AS categoria_turismo, 
                  NOW() AS fecha_inicio, 
                  NOW() AS fecha_fin, 
              ...
              WHERE actividades.estado = true", [$idIdioma]);
            // $query = DB::select("(SELECT actividades.id AS id,  
            //      actividades.calificacion_legusto AS calificacion_legusto, 
            //      1 AS tipo, 
            //      NOW() AS fecha_inicio, 
