...
class ImportacionRntController extends Controller
{
	public function __construct()
    {
        ...
        $this->middleware('permissions:importar-proveedores');
        if(Auth::user() != null){
    ...
    public function postCargarsoporte(Request $request){
		...
		$municipios = Municipio::where('departamento_id',1411)->select(\DB::raw('cambia_caracter(upper(nombre)) as nombre, id'))->get();
		...
		foreach($nuevos as $registro){
		    ...
		    $similar = $proveedoresIngresados->filter(function($value, $key)use($registro){
		    	return ($value['nit'] == $registro['nit'] && $registro['nit'] != null && $registro['nit'] != 0) || ( $value['digito_verificacion'] == $registro['digito_verificacion'] && $value['digito_verificacion'] != 0 && $value['digito_verificacion'] != null && ($value['nit'] == $registro['nit'] && $registro['nit'] != null && $registro['nit'] != 0) ) || ($value['correo'] == $registro['correo'] && strpos($value['correo'], '@') !== false);
		    })->first();
