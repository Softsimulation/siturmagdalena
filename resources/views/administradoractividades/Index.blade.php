    ...
    <div class="blank-page widget-shadow scroll" id="style-2 div1">
        <div class="row" style="margin: 0;">
            <div class="col-xs-12 col-sm-6 col-md-5">
                <a href="/administradoractividades/crear" type="button" class="btn btn-primary" >
                  Insertar actividad
                </a>
            </div>
            <div class="col-xs-12 col-sm-4 col-md-4">
                <input type="text" ng-model="prop.search" class="form-control" id="inputEmail3" placeholder="Búsqueda de actividades">
            </div>
            <div class="col-xs-12 col-sm-2 col-md-3" style="text-align: center;">
                <span class="chip">@{{(actividades|filter:prop.search).length}} resultados</span>
            </div>
        </div>
        <br/>
        <div class="row">
            <div class="col-xs-12">
                <ul class="media-list">
                    <li dir-paginate="actividad in actividades | filter:prop.search | itemsPerPage:10" pagination-id="pagination_actividades" class="media">
                        <div class="media-left">
                            <a href="/administradoractividades/editar/@{{actividad.id}}">
                                <img class="media-object" style="width: 400px; height: 200px;" 
                                src="@{{actividad.multimedias_actividades.length > 0 ?  actividad.multimedias_actividades[0].ruta : 'img/app/noimage.jpg'}}" 
                                alt="@{{actividad.actividades_con_idiomas[0].nombre}}">
                            </a>
                        </div>
                        <div class="media-body">
                            <h4 class="media-heading">@{{actividad.actividades_con_idiomas[0].nombre}}</h4>
                            <p class="text-justify">
                                @{{actividad.actividades_con_idiomas[0].descripcion | limitTo:400}}...
                            </p>
                            <br>
                            <p class="text-left">
                                ...
                                <a href="/administradoractividades/idioma/@{{actividad.id}}/@{{traduccion.idioma.id}}" ng-repeat="traduccion in actividad.actividades_con_idiomas"> @{{traduccion.idioma.culture}}</a>
                                <a href="javascript:void(0)" ng-click="modalIdioma(actividad)" ng-if="actividad.actividades_con_idiomas.length < idiomas.length"> <span class="glyphicon glyphicon-plus"></span></a>
                                <a href="/administradoractividades/editar/@{{actividad.id}}"> <span class="glyphicon glyphicon-pencil"></span></a>
                            </p>
                        </div>
                    </li>
                </ul>
                <div class="alert alert-warning" role="alert" ng-show="actividades.length == 0 || (actividades|filter:prop.search).length == 0">No hay resultados disponibles <span ng-show="(actividades|filter:prop.search).length == 0">para la búsqueda '@{{prop.search}}'. <a href="#" ng-click="prop.search = ''">Presione aquí</a> para ver todos los resultados.</span></div>
            </div>
