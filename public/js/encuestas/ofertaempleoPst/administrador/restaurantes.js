var situr = angular.module("adminPst.restaurante", []);

situr.controller('caracterizacionAlimentosCtrl', ['$scope','restauranteServiPst', function ($scope,restauranteServiPst) {

    $scope.alimentos = {}
    $scope.$watch('id', function () {
        $("body").attr("class", "charging");    
        restauranteServiPst.getInfoAlimentosC($scope.id).then(function (dato) {
    ...
    $scope.guardar = function () {
        ...
        $("body").attr("class", "charging"); 
        restauranteServiPst.GuardarCarAlimentos($scope.alimentos).then(function (data) {
            $("body").attr("class", "cbp-spmenu-push");
            if (data.success) {
                swal({
                  title: "Realizado",
...
situr.controller('capacidadAlimentosCtrl', ['$scope','restauranteServiPst', function ($scope,restauranteServiPst) {

    $scope.alimentos = {}

    $scope.$watch('id', function () {
        $("body").attr("class", "charging");
        restauranteServiPst.getInfoCapAlimentos($scope.id).then(function (data) {
    ...
    $scope.guardar = function () {
        ...
        $("body").attr("class", "charging");
        restauranteServiPst.GuardarOfertaAlimentos($scope.alimentos).then(function (data) {
            $("body").attr("class", "cbp-spmenu-push");
            if (data.success) {
                    swal({
                  title: "Realizado",
