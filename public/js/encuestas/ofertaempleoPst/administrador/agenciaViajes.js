var situr = angular.module("adminPst.agenciaViaje", []);

situr.controller('caracterizacionAgenciaViajesCtrl', ['$scope','agenciaViajeServiPst', function ($scope,agenciaViajeServiPst) {
    $scope.encuesta = {};
    ...
    $scope.$watch('encuesta.id', function () {
        ...
        if ($scope.encuesta.id != null) {
            ...
            agenciaViajeServiPst.getAgencia($scope.encuesta.id).then(function (data) {
    ...
    $("body").attr("class", "charging");
    agenciaViajeServiPst.getDatosAgencia().then(function (data) {
    ...
    $scope.guardar = function () {
       ...
       if ($scope.DatosForm.$valid && $scope.encuesta.TipoServicios.length>0) {
            ...
            $("body").attr("class", "charging");
            agenciaViajeServiPst.guardarCaracterizacion($scope.encuesta).then(function (data) {
                $("body").attr("class", "cbp-spmenu-push");
                if (data.success) {
...
situr.controller('ofertaAgenciaViajesCtrl', ['$scope','agenciaViajeServiPst', function ($scope,agenciaViajeServiPst) {
    $scope.encuesta = {};
    ...
    $scope.$watch('encuesta.id', function () {
        ...
        if ($scope.encuesta.id != null) {
            $("body").attr("class", "charging");
            agenciaViajeServiPst.getOfertaAgencia($scope.encuesta.id).then(function (data) {
                $scope.proveedor = data.proveedor;
                if (data.agencia.id > 0) {
    ...
    $("body").attr("class", "charging");
    agenciaViajeServiPst.getDatosOfertaAgencia().then(function (data) {
        $scope.destinos = data;
        $("body").attr("class", "cbp-spmenu-push");
    ...
    $scope.guardar = function () {
       ...
       if ($scope.DatosForm.$valid) {
            ...
            agenciaViajeServiPst.guardarOfertaAgenciaViajes($scope.encuesta).then(function (data) {
                $("body").attr("class", "cbp-spmenu-push");
                if (data.success) {
