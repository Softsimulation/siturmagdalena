...
.controller('misFavoritosCtrl', ['$scope', 'visitanteServi','$filter',function ($scope, visitanteServi,$filter) {
    ...
    $scope.ordenarItem = function (item, items) {
        var anterior = $filter('filter')(items, {'Orden': (item.Orden - 1) }, true);
        anterior[0]['Orden'] +=  1;
        item.Orden--;
    }
    ...
    $scope.eliminarPlanificador = function(planificador){
        
        var indexEliminar = $scope.listaPlanificadores.indexOf(planificador);
        ...
        function () {
            setTimeout(function () {
                $("body").attr("class", "charging");
                visitanteServi.EliminarPlanificador(planificador).then(function(data){
                    if(data.success){
                        $scope.listaPlanificadores.splice(indexEliminar,1);
                        swal({
                            title: "Planificador eliminado",
                            text: "Se ha eliminado el planificador satisfactoriamente.",
                            type: "success",
                            timer: 1000,
...
.controller('editarPlanificadorCtrl', ['$scope', 'visitanteServi','$filter',function ($scope, visitanteServi,$filter) {
    ...
    $scope.$watch('Id', function () {
        if ($scope.Id != null) {
            $("body").attr("class", "charging");
            visitanteServi.CargarEditarPlanificador($scope.Id).then(function(data) {
               $scope.favoritos = data.favoritos;
               $scope.listaPlanificadores = data.listaPlanificadores;
               $scope.planificador = data.planificador;
               $("body").attr("class", "cbp-spmenu-push");
    ...
    $scope.abrirEditarPlanificador = function(){
        $scope.nuevoPlanificador = angular.copy($scope.planificador);
        $scope.editarForm.$setPristine();
        $scope.editarForm.$setUntouched();
        $scope.editarForm.$submitted = false;
        $('#modalEditarPlanificador').modal('show');
    ...
    $scope.editarPlanificador = function(){
        ...
        var dateOutRange = false;
        for (var i = 0; i < $scope.listaPlanificadores.length; i++) {
            if (! validarFechas( stringToDate($scope.nuevoPlanificador.Fecha_inicio) , stringToDate($scope.nuevoPlanificador.Fecha_fin) , stringToDate(($scope.listaPlanificadores[i].Fecha_inicio)) , stringToDate($scope.listaPlanificadores[i].Fecha_fin) ) ) {
                dateOutRange = true;
                break;
            }
        }
        ...
        $scope.planificador.Nombre = $scope.nuevoPlanificador.Nombre;
        $scope.planificador.Fecha_inicio = $scope.nuevoPlanificador.Fecha_inicio;
        $scope.planificador.Fecha_fin = $scope.nuevoPlanificador.Fecha_fin;
        $('#modalEditarPlanificador').modal('hide');
    }
    
    $scope.guardar = function(){
        
        if($scope.planificador.Dias.length == 0){
            swal("Error", "Para guardar un planificador debe verificar que este tenga por lo menos un día.", "error");
            return;
        }
        
        $("body").attr("class", "charging");
        visitanteServi.EditarPlanificador($scope.planificador).then(function(data) {
            if(data.success) {
                swal("Realizado", "Se ha guardado satisfactoriamente el planificador.", "success");
                window.location = '/visitante/misfavoritos';
            } else {
